...
class DefaultGenerator implements GeneratorInterface
{
    ...
    static function pack($data, $compress = true)
    {
        ...
        foreach (str_split($data, $max) as $part) {
            $length = strlen($part);
            $parts[] = pack(sprintf('LCA%s', $length), $length, self::crc8($part), $part);
        }
    ...
    static function unpack($data)
    {
        ...
        while (false === feof($handler)) {
            $chunkHeader  = fread($handler, 5);
            $chunkHeader  = unpack('Lsize/Ccrc', $chunkHeader);
            if ($chunkHeader['size']) {
                $data = fread($handler, $chunkHeader['size']);
                $crc = self::crc8($data);
                if ($crc !== $chunkHeader['crc']) {
                    throw new \RuntimeException(sprintf('CRC check failed, data is incomplete or corrupt (0x%02x != 0x%02x)', $crc, $chunkHeader['crc']));
                }
                $return .= $data;
            }
        }
    ...
    /**
     * generate a 8 bit crc from given data
     *
     * @url http://stackoverflow.com/questions/14079444/how-to-generate-8bit-crc-in-php/14081891#14081891
     *
     * @param   $data
     * @return  int
     */
    static function crc8($data)
    {
        $table = [
